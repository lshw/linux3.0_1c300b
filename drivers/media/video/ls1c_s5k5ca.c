...
static int thread_func(void *data)
{
	while (1)
	{
		...
		unsigned int temp = 0;
		while (1) {
		temp = ((*(volatile unsigned int*)(0xbc280038)) & 0xf00000) >> 20;
		if (0x0e == temp)
		 {
			eNum++;
			temp = ((*(volatile unsigned int*)(0xbc280038)) & 0xf00000) >> 20;
		...
		//printf("");
		if (eNum)
			printk("bit20-23 eNum %d \n", eNum);
		//eNum = 0;
		//printf("config reg 0d%ld 0x%x \n", count++, *(volatile unsigned int*)(0xbc280038));
		}
...
static int start_bit28_thread(void)
{
	_task = kthread_run(thread_func, NULL, "thread");
	if (!IS_ERR(_task))
