...
static int ls1x_xfer_read(struct ls1x_i2c *i2c, unsigned char *buf, int length) 
{
	...
	for (x=0; x<length; x++) {
		...
		if (x != (length -1)) 
			i2c_writeb(i2c, OCI2C_CMD, OCI2C_CMD_READ_NACK); //dbg-yg nack for camera
		else
...
//	i2c_writeb(i2c, OCI2C_CONTROL, 0);	//lxy
...
#else
	i2c_writeb(i2c, OCI2C_PRELOW, 0x64);
	i2c_writeb(i2c, OCI2C_PREHIGH, 0);
#endif

	/* Init the device */
	i2c_writeb(i2c, OCI2C_CMD, OCI2C_CMD_IACK);
	i2c_writeb(i2c, OCI2C_CONTROL, ctrl | OCI2C_CTRL_IEN | OCI2C_CTRL_EN);
//	i2c_writeb(i2c, OCI2C_CONTROL, OCI2C_CTRL_EN);	//lxy
}
