...
#define	CONFIG_PARA_OUTPUT_DATA_MODE	13
...
//	printk("-----stop 0x%x-----\n", temp);
	 ...
	 */
	videobuf_waiton(vq, vb, 0, 0);   //dbg-yg-ques
...
//	printk("UTC time :%d-%d-%d %d:%d:%d \n",tm.tm_year+1900,tm.tm_mon, tm.tm_mday,tm.tm_hour,tm.tm_min,tm.tm_sec);
	...
	unsigned long flags;
	//把camera 缓冲区地址 设置为 lcd framebuffer dma base
	temp = __raw_readl(pcdev->base + CAM_DMA0_CONFIG);
	__raw_writel(temp,  0xbc301260 ); //  lcd  dma base
//

	temp = __raw_readl(pcdev->base + CAM_CAMIF_CONFIG);
	while (temp & 0x40000000) {            //if bit:30 == 1 ,dma is in idle 
		temp = __raw_readl(pcdev->base + CAM_CAMIF_CONFIG);
		temp |= (1 << 29);
...
static int camera_thread(void *pthread_data)
{
	...
	while (1) {
		...
		vb = &pcdev->active->vb;
		buf = container_of(vb, struct ls1c_camera_buffer, vb); // <==> buf = pcdev->active 
...
static int ls1c_camera_set_bus_param(struct soc_camera_device *icd, __u32 pixfmt)
{
	...
	if (icd->user_width == 640)
		csicr1 |=  (1 << CONFIG_PARA_640X480) ;

	csicr1 |= (1 << CONFIG_PARA_INPUT_DATA_MODE)  ;//yuyv intput (bt.601)
	csicr1 |= (1 << CONFIG_PARA_OUTPUT_DATA_MODE)  ;//rgb  output
	__raw_writel(csicr1, pcdev->base + CAM_CAMIF_CONFIG);


//	__raw_writel((LCD_WIDTH - icd->user_width)*2, pcdev->base + CAM_UOFF_CONFIG); //offset
	__raw_writel(320, pcdev->base + CAM_UOFF_CONFIG); //offset
