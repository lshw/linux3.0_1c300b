...
static void set_graphics_start(struct fb_info *info, int xoffset, int yoffset)
{
	...
	printk("---------set_graphics_start 0x%x-----------\n", addr);
	printk("frame buffer addr 0x%x \n", fbi->reg_base + LS1X_FB_ADDR0);
...
static int __devexit ls1xfb_remove(struct platform_device *pdev)
{
