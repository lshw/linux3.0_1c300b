...
s32 synopGMAC_mac_init(synopGMACdevice * gmacdev)
{
	...
	if(gmacdev->DuplexMode == FULLDUPLEX){
		...
		synopGMACSetBits((u32 *)gmacdev->DmaBase,DmaControl, DmaStoreAndForward);//3
		synopGMACSetBits((u32 *)gmacdev->DmaBase,DmaControl, DmaFwdErrorFrames);
		synopGMACSetBits((u32 *)gmacdev->DmaBase,DmaControl, 0x02000000);
		...
		else
		{
			...
			if(gmacdev->Speed == SPEED100)
				synopGMACSetBits((u64 *)gmacdev->MacBase, GmacConfig, GmacFESpeed100);
			else
				synopGMACSetBits((u64 *)gmacdev->MacBase, GmacConfig, GmacFESpeed10);
		}
		...
		synopGMACSetBits((u64 *)gmacdev->DmaBase,DmaControl, DmaStoreAndForward);
		synopGMACSetBits((u64 *)gmacdev->DmaBase,DmaControl, DmaFwdErrorFrames);
		synopGMAC_tx_enable(gmacdev);	
		synopGMAC_rx_enable(gmacdev);
...
/*
phy 88e1111 rx/tx phase init.
*/

s32 synopGMAC_phy88e1111_phase_init (synopGMACdevice * gmacdev) 
{
	u16 data;
	s32 status = -ESYNOPGMACNOERR;		

		status = synopGMAC_read_phy_reg((u32 *)gmacdev->MacBase,gmacdev->PhyBase,PHY_EXT_PHY_SPC_CTRL	 , &data);
	data = data | 0x0082;
		status = synopGMAC_write_phy_reg((u32 *)gmacdev->MacBase,gmacdev->PhyBase,PHY_EXT_PHY_SPC_CTRL	, data);
		status = synopGMAC_read_phy_reg((u32 *)gmacdev->MacBase,gmacdev->PhyBase,0x0	 , &data);
	data = data | 0x8000;
		status = synopGMAC_write_phy_reg((u32 *)gmacdev->MacBase,gmacdev->PhyBase,0x0	, data);
	return status;
}
...
/**
  ...
  * \return returns void.
  */
void synopGMAC_get_desc_data(DmaDesc * desc, u32 * Status, dma_addr_t * Buffer1, u32 * Length1, u64 * Data1, dma_addr_t * Buffer2, u32 * Length2, u64 * Data2)
{
...
/**
  ...
  * \return returns present tx descriptor index on success. Negative value if error.
  */
s32 synopGMAC_set_tx_qptr(synopGMACdevice * gmacdev, u32 Buffer1, u32 Length1, u64 Data1, u32 Buffer2, u32 Length2, u64 Data2,u32 offload_needed)
{
...
s32 synopGMAC_set_rx_qptr(synopGMACdevice * gmacdev, u32 Buffer1, u32 Length1, u64 Data1, u32 Buffer2, u32 Length2, u64 Data2)
{
	...
	TR("%02d %p %08x %08x %08x %08x %lx %lx\n",rxnext,rxdesc,rxdesc->status,rxdesc->length,rxdesc->buffer1,rxdesc->buffer2,rxdesc->data1,rxdesc->data2);
...
s32 synopGMAC_set_rx_qptr(synopGMACdevice * gmacdev, u32 Buffer1, u32 Length1, u64 Data1, u32 Buffer2, u32 Length2, u64 Data2)
{
	...
	TR("<0>synopGMAC_set_rx_qptr length=%x\n", rxdesc->length );
	if(!synopGMAC_is_desc_empty(rxdesc))
        {
        TR("not empty\n");
		return -1;
	...
	if(synopGMAC_is_rx_desc_chained(rxdesc)){
		...
		gmacdev->RxNext     = synopGMAC_is_last_rx_desc(gmacdev,rxdesc) ? 0 : rxnext + 1;
		gmacdev->RxNextDesc = synopGMAC_is_last_rx_desc(gmacdev,rxdesc) ? gmacdev->RxDesc : (rxdesc + 1);
                TR("<0>RxNext=%x,RxNextDesc=%p\n", gmacdev->RxNext,gmacdev->RxNextDesc);
	}
	TR("%02d %p %08x %08x %08x %08x %lx %lx\n",rxnext,rxdesc,rxdesc->status,rxdesc->length,rxdesc->buffer1,rxdesc->buffer2,rxdesc->data1,rxdesc->data2);
	(gmacdev->BusyRxDesc)++; //One descriptor will be given to Hardware. So busy count incremented by one
	return rxnext;
...
u32 synopGMAC_get_interrupt_type(synopGMACdevice *gmacdev)
{
	...
	u32 interrupts = 0;
	data = synopGMACReadReg((u32 *)gmacdev->DmaBase, DmaStatus);
	synopGMACWriteReg((u32 *)gmacdev->DmaBase, DmaStatus ,data); //manju: I think this is the appropriate location to clear the interrupts
