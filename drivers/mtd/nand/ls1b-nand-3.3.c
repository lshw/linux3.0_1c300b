...
extern mtd_device_parse_register(struct mtd_info *mtd, const char **types,
...
{
    return(*((volatile unsigned int*)0xbfe78000) & (0x1<<16));
}
...
//                info->coherent = 0;
                if(info->state != STATE_BUSY){
                    printk("nandflash chip ops error...\n");
                    return;
                }
//                info->state = STATE_BUSY;
...
static int __init ls1b_nand_init(void)
{
    ...
    *((volatile unsigned int *)0xbfd00420) = 0x14000000;
    ...
    if(ret){
        printk(KERN_ERR "failed to register ls1b_1g_nand_driver");
    }
    return ret;
}
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("ls1b_1a NAND controller driver");
