...
*/
/* I2C devices fitted. */
#ifdef CONFIG_TOUCHSCREEN_TSZT2083
#define TSZT2083_GPIO_IRQ	32
static int ts_get_pendown_state(void)
{
	return !gpio_get_value(TSZT2083_GPIO_IRQ);
}

int ts_init(void)
{
	gpio_request(TSZT2083_GPIO_IRQ, "tszt2083 gpio irq");
	gpio_direction_input(TSZT2083_GPIO_IRQ);
	return 0;
}
#
...
static struct i2c_board_info __initdata ls1x_i2c0_devs[] = {
#ifdef CONFIG_TOUCHSCREEN_TSZT2083
	{
		...
		.irq = LS1X_GPIO_FIRST_IRQ + TSZT2083_GPIO_IRQ,
...
#ifdef CONFIG_LS1X_GMAC0
static u64 ls1a_gmac_dmamask = DMA_BIT_MASK(32);
static struct resource ls1x_gmac0_resources[] = { 
 [0] = {
   .start          = LS1X_GMAC0_BASE,
   .end            = (LS1X_GMAC0_BASE + 0x6b),
   .flags          = IORESOURCE_MEM,
 },
 [1] = {
   .start          = LS1X_GMAC0_IRQ,
...
static struct platform_device ls1x_gmac0_device = {
 .name           = "loongson-gmac",
 .id             = 1,
 .dev = {
   .dma_mask=&ls1a_gmac_dmamask,
 },
 .num_resources  = ARRAY_SIZE(ls1x_gmac0_resources),
 .resource       = ls1x_gmac0_resources,
};
#endif
...
#ifdef CONFIG_SOUND_LS1X_UDA1342
static struct resource ls1x_uda1342_resource[] = {
	[0]={
		.start	= LS1X_AC97_BASE,
		.end	= LS1X_AC97_BASE + SZ_16K - 1,
		.flags	= IORESOURCE_MEM,
...
static struct platform_device ls1x_audio_device = {
	.name           = "ls1x-audio",
	.id             = -1,
	.num_resources	= ARRAY_SIZE(ls1x_uda1342_resource),
	.resource		= ls1x_uda1342_resource,
};
#endif
...
static struct spi_board_info spi_gpio_devices[] = {
	{
		.modalias	= "spidev",
		.bus_num 		= 0,	/* 对应spigpio_device的.id=2 */
...
#endif
/* * *  ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^  SDIO DEVICE  ^^^^^^^^^^^^^^^^^^^^^^^ */
#ifdef  CONFIG_MMC_LS1C
static u64 ls1c_sdio_dma_mask = DMA_BIT_MASK(32);
static struct resource ls1c_sdio_resources[] = { 
	[0] = {
		.start = LS1C_SDIO_REG_BASE,
		.end   = (LS1C_SDIO_REG_BASE + 0x7fff),
		.flags = IORESOURCE_MEM,
	},
	[1] = {
		.start = LS1C_SDIO_IRQ,
...
static struct platform_device ls1c_sdio_device = {
	.name           = "ls1c_sdio",
	.id             = 0,
	.dev = {
		.dma_mask = &ls1c_sdio_dma_mask,
		.coherent_dma_mask	= DMA_BIT_MASK(32),
	},
	.num_resources  = ARRAY_SIZE(ls1c_sdio_resources),
	.resource       = ls1c_sdio_resources,
};
#endif
#ifdef	CONFIG_SOC_CAMERA_LS1C
#if 0
...
static int ls1c_camera_add(struct soc_camera_link *icl, struct device *dev);
static void ls1c_camera_del(struct soc_camera_link *icl);

static struct soc_camera_link camera_link = {
	.bus_id		= 0,
	.add_device	= ls1c_camera_add,
	.del_device	= ls1c_camera_del,
...
static int ls1c_camera_add(struct soc_camera_link *icl,
			       struct device *dev)
{
	int ret = soc_camera_platform_add(icl, dev, &camera_device, &camera_link,
					  ls1c_camera_release, 0);
	if (ret < 0)
		return ret;
...
static struct i2c_board_info ov9650_i2c_camera = {
	I2C_BOARD_INFO("OV9650", 0x60>>1),  // i2c addr
};

static struct soc_camera_link ov9650_link = {
	.bus_id         = 0,
	.i2c_adapter_id = 0,
	.board_info     = &ov9650_i2c_camera,
//	.power          = ls1c_camera_power,
//	.reset          = ls1c_camera_reset,
};
#endif
...
static struct platform_device ls1a_ac97_device = {
        .name           = "ls1a-ac97",
        .id             = -1,
};
...
static struct platform_device ls1a_codec = {
        .name           = "uda134x-codec",
        .id             = -1,
};
...
#ifdef CONFIG_SOUND_LS1X_UDA1342  //new
	&ls1x_audio_device,
#endif
...
int __init ls1c_platform_init(void)
{
	ls1x_serial_setup();
