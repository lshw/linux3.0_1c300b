...
#include "ls1a-pcm.h"//For DMA Reasons and AC97 Registers
#include "../soc/loongson/ls1a-pcm.h"//For sth shared with ls1a-ac97.c
				...
				struct snd_pcm_hw_params *params)
{
	...
	do {
		...
		dma_desc->daddr = rtd->params->dev_addr;		/// right!
...
int ls1a_pcm_preallocate_dma_buffer(struct snd_pcm *pcm, int stream)
{
	...
	buf->area = dma_alloc_coherent(pcm->card->dev, size,
					   &buf->addr, GFP_KERNEL);
	if (!buf->area){			printk("func %s,line%i\n",__func__,__LINE__);
