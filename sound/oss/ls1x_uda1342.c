...
//	dump_stack();
	if (s->ask_dma)
		...
		printk("fsy alloc size ==%d\n",s->nbfrags);
		printk("fsy alloc size ==%d\n",s->fragsize);
	for (i=0; i <s->nbfrags; i++) {
		struct audio_dma_desc *desc;
		
		j = sizeof(struct audio_dma_desc);
//		printk("fsy alloc size ==%d\n",j);
//		desc = dma_alloc_coherent(NULL, s->nbfrags * sizeof(struct audio_dma_desc),
...
//	for (i=0;i<s->nbfrags;i++){
//		printk("3. alloc dma desc err %x    %p.\n",dma_phyaddr,desc);
//		memset(desc, 0, s->nbfrags * sizeof(struct audio_dma_desc));
		...
		j = get_order(s->fragsize);
//		printk("fsy alloc num ==%d\n",j);
...
//		desc = desc + sizeof(struct audio_dma_desc);
//		dma_phyaddr = dma_phyaddr + sizeof(struct audio_dma_desc);
	}
...
static long ls1x_audio_ioctl(struct file *file, uint cmd, ulong arg)
{
	...
	case SNDCTL_DSP_SETFRAGMENT:
		...
		printk("fsy alloc size1 ==%d\n",os->fragsize);
		printk("fsy alloc size 2==%d\n",os->nbfrags);
		printk("fsy alloc size1 ==%d\n",is->fragsize);
		printk("fsy alloc size 2==%d\n",is->nbfrags);
		if (rd) {
			is->fragsize = 1 << (val & 0xFFFF);
			if (is->fragsize < 1024) is->fragsize = 1024;
			if (is->fragsize > 1024) is->fragsize = 1024;//fsy
			is->nbfrags = (val >> 16) & 0xFFFF;
			if (is->nbfrags < 12) is->nbfrags = 12;
		}

		if (wr) {
			os->fragsize = 1 << (val & 0xFFFF);
			if (os->fragsize < 1024) os->fragsize = 1024;
			if (os->fragsize > 1024) os->fragsize = 1024;//fsy
			os->nbfrags = (val >> 16) & 0xFFFF;
			if (os->nbfrags < 12) os->nbfrags = 12;
			if (os->nbfrags > 12) os->nbfrags = 12;
//			if (os->num_channels) {
//				//os->fragsize >>= 1;
//				os->nbfrags >>= 2;
...
static int ls1x_audio_probe(struct platform_device *pdev)
{
	...
	if ((state->dev_mixer = register_sound_mixer(&ls1x_mixer_fops, -1)) < 0){
		printk("error !\n");
		goto err_dev1;		
	}
